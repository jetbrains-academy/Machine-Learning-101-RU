...
def centroid_histogram(labels):
    # Here we retrieve the counts of the unique entrances in the
    # labels array (those are the clusters, we need to know how many
...
def plot_colors(hist, centroids):
    ...
    # the histogram. It is scaled to have width of 500
    ...
    # The x-axis position we start to draw at
    start_x = 0
    # The overall sum of the samples belonging to all of the clusters
    sum_hist = np.sum(hist)
    for (percent, color) in zip(hist, centroids):
        # The end x-axis position is obtained by adding a percentage
        # of all cluster points from the overall count,
        # scaled to have a width 500,
        # to the starting position
        end_x = #TODO
        # Here we draw current rectangle with a height of 50 and color
